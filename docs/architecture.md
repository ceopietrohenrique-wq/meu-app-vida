...
| Exportação/backup 100% client-side, sem rota server-side nova | `domains/export/`, `domains/finance/services/export-service.ts`, `domains/sales/services/export-service.ts` chamam o Supabase client autenticado normal (RLS via `auth.uid()`), nunca `service_role`, nunca um Route Handler novo | RLS já garante isolamento por usuário do mesmo jeito que qualquer leitura existente do app — uma rota server-side só duplicaria essa proteção sem ganho real (CLAUDE.md > Simplicidade acima de Escalabilidade); paginação (`fetchAllRows`, lotes de 1000 via `.range()`) evita truncar silenciosamente um histórico grande |
| `computeGrossProfitCents`/`computeMarginPercent` movidas para `domains/sales/utils/` | `business-indicators.ts` reexporta de `sales/utils/sale-indicators.ts` — API pública inalterada, nenhum import existente quebra | auditoria de exportação: o CSV de vendas (domínio `sales`) precisava da mesma fórmula que só existia em `business/utils` — `sales` é o dono natural do conceito (calculado a partir de campos da própria tabela `sales`); mesmo padrão de promoção já usado para `shared/lib/streak.ts` quando 2+ consumidores precisam do mesmo cálculo puro |
